...
public class ShotgunFactory extends AbstractUnitFactory {

	private static int maxHP;
	private static int damage;
	private static int movementRange;
	
	public ShotgunFactory() {
		this.unittype = UNITTYPE.SHOTGUN;
	}
	...
	public Unit create(Vector2 position, boolean belongsToPlayerOne) {
		...
		unit.setName(UNITTYPE.SHOTGUN.toString());
		return unit;
	}
	
	public static void setParameters(int damage, int health, int movementRange) {
		ShotgunFactory.damage = damage;
		ShotgunFactory.maxHP = health;
		ShotgunFactory.movementRange = movementRange;		
	}
	...
	private void addComponents(Vector2 position, Unit unit){
		...
		TextureRegion region = ContentLoader.getInstance().getUnitTexture(UNITTYPE.SHOTGUN, ANIMATIONTYPE.IDLE , unit.isOwnedByPlayerOne());
