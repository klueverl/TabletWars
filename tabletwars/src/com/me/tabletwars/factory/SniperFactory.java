...
public class SniperFactory extends AbstractUnitFactory {
	...
	public Unit create(Vector2 position, boolean belongsToPlayerOne) {
		...
		unit.setUnitType(UNITTYPE.SNIPER);
		unit.setMovementRange(movementRange);
		addComponents(position, unit);
		unit.setName(UNITTYPE.SNIPER.toString());
		return unit;
	}

	public static void setParameters(int damage, int health, int movementRange) {
		SniperFactory.damage = damage;
		SniperFactory.movementRange = movementRange;
		SniperFactory.maxHP = health;
	}
	...
	private void addComponents(Vector2 position, Unit unit){
		...
		TextureRegion region = ContentLoader.getInstance().getUnitTexture(UNITTYPE.SNIPER, ANIMATIONTYPE.IDLE , unit.isOwnedByPlayerOne());
