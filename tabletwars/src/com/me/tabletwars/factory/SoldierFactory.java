...
public class SoldierFactory extends AbstractUnitFactory {
	...
	public Unit create(Vector2 position, boolean belongsToPlayerOne) {
		...
		unit.setUnitType(UNITTYPE.SOLDIER);
		unit.setMovementRange(movementRange);
		addComponents(position, unit);
		unit.setName(UNITTYPE.SOLDIER.toString());
		return unit;
	}
	...
	private void addComponents(Vector2 position, Unit unit){
		...
		TextureRegion region = ContentLoader.getInstance().getUnitTexture(UNITTYPE.SOLDIER, ANIMATIONTYPE.IDLE , unit.isOwnedByPlayerOne());
	...
	public static void setParameters(int damage, int health, int movementRange) {
		SoldierFactory.maxHP = health;
		SoldierFactory.damage = damage;
		SoldierFactory.movementRange = movementRange;		
	}
}
